...
@attrs
class AddTaskParams(object):
    name = attrib(default="add-task")
...
#
# To create a network simply instantiate the `:class:AddTaskModelTraining`,
# all the components will be wired with the default values.
# In case you'd like to change any of defaults, do the following:
#
# > params = AddTaskParams(batch_size=4)
# > model = AddTaskModelTraining(params=params)
#
...
# for evaluating.
#
# You may skip this alltogether, and use `:class:AddTaskNTM` directly.
#

@attrs
class AddTaskModelTraining(object):
    params = attrib(default=Factory(AddTaskParams))
    net = attrib()
    dataloader = attrib()
    criterion = attrib()
    optimizer = attrib()
